...
PTAX_CHALLAN_BASE_URL = "http://ec2-18-221-89-14.us-east-2.compute.amazonaws.com/flaskapp/p_tax_challan/pTaxChallan/"
PTAX_PAYMENT_BASE_URL = "http://ec2-18-221-89-14.us-east-2.compute.amazonaws.com/flaskapp/p_tax_payment/pTaxPayment/"
...
if __name__ == '__main__':
	...
	user_id = 6
	get_uploading_track_query = ("""SELECT * from `uploading_track` WHERE `last_update_id` = %s and `status` = 1 and `uploading_type` = 'p-tax' """)
				...
				for uploading_track_details_key,uploading_track_details_data in enumerate(uploading_track_details_data):					
					...
					if checkKeyIsExsits_response['responseList']['get_key'] == 1:	
						...
						if uploading_track_details_data['uploading_data_into_database'] == 0:
							...
							print(readFileAndSaveData)
							if data['content_type'] == 'challan':
								readFileAndSaveDataUrl = PTAX_CHALLAN_BASE_URL + "readFromFileDataAndSave"
							else:
								readFileAndSaveDataUrl = PTAX_PAYMENT_BASE_URL + "readFromFileDataAndSave"
